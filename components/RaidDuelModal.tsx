...
import { toast } from '../lib/toast' // you already have the Toasts system
...
const QUIPS = [
  'Spinning up proxies…','Bribing the firewall…','Decrypting memes…',
  'Spoofing MAC & cheese…','Engaging stealth socks…','Feeding the AI pizza…',
  'Minions are arguing…','Tuning packet vibes…','Hacking the planet…'
]
...
export default function RaidDuelModal({
  ...
  const attack = useMutation({
    ...
    onSuccess: (r) => {
      ...
      setStage('result')
      if (r.win) toast.success('You won the raid!', `+${r.xp} XP · +${r.coins} coins`)
      else toast.warn('You lost the raid', `+${r.xp} XP · +${r.coins} coins`)
            ...
            {stage === 'intro' && (
              <div className="mt-6 text-center">
                ...
                <div className="opacity-80 text-sm mt-1">
                  L{target.level ?? '?'} • {target.batch ?? '8B'} • Power {target.power ?? '—'}
                </div>
            ...
            {stage === 'result' && res && (
              <div className="mt-6">
                <div className="text-center font-heading text-2xl mb-2 gradient-text">
                  {res.win ? 'Victory!' : 'Defeated…'}
                </div>
                <div className="text-center opacity-90">+{res.xp} XP · +{res.coins} coins</div>
