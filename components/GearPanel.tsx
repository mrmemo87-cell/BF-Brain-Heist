  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      <section className="glassmorphism rounded-2xl p-4">
        ...
        {shop.isLoading ? <div className="opacity-70 text-sm">Loading…</div> : (
            ...
            {shop.data!.map(it => (
              <div key={it.id} className="rounded-xl border p-3 flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xs opacity-70">Slot: {it.slot} • ATK {it.atk} • DEF {it.def}</div>
                ...
                <div className="text-right">
                  ...
                  >
                    {it.affordable ? (buy.isPending ? 'Buying…' : 'Buy') : 'Too pricey'}
                  </button>
      ...
      <section className="glassmorphism rounded-2xl p-4">
        ...
        {inv.isLoading ? <div className="opacity-70 text-sm">Loading…</div> : (
            ...
            {inv.data!.map(it => (
              <div key={it.userItemId} className="rounded-xl border p-3 flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xs opacity-70">ATK {it.atk} • DEF {it.def}</div>
                ...
                >
                  {equip.isPending ? 'Updating…' : (it.equipped ? 'Unequip' : 'Equip')}
                </button>
              </div>
            ))}
          </div>
