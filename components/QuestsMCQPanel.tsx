  ...
  const submit = useMutation({
    ...
    onSuccess: async (res: any) => {
      if ((res as any).correct) {
        toast.success('Correct!', `+${(res as any).xp} XP · +${(res as any).coins} coins`)
        await supa.rpc('news_add', {
          p_kind: 'quest',
          p_title: 'MCQ complete',
          p_body: `+${(res as any).xp} XP · +${(res as any).coins} coins`
        })
        Audio.play('success');
      } else {
        toast.warn('Almost…', 'Try the next one!')
        Audio.play('fail')
      }
          ...
          {next.isLoading ? (
            ...
            <motion.div key="done" initial={{opacity:0, y:10}} animate={{opacity:1, y:0}} exit={{opacity:0, y:-10}}>
              <div className="text-center py-10">
                <div className="font-heading text-lg mb-2">You’re all caught up 🎉</div>
