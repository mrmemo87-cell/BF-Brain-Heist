...
export const supaAPI: DataAPI = {
  // Load the current user's profile (no demo fallback)
  async whoAmI(): Promise<Profile | null> {
    ...
    return { ...data, avatarUrl: (data as any).avatar_url }
  },
  ...
  async login(identifier: string, password?: string) {
    ...
    const { error } = await supa.auth.signInWithOtp({
      email: identifier,
      options: { emailRedirectTo: window.location.origin } // ok to land on "/"
    })
  ...
  async logout() {
    try {
      ...
      await supa.auth.signOut({ scope: 'local' as const }); // clear tokens even if network fails
    }
  },
  ...
  async raidAttack(defenderId: string): Promise<BattleResult> {
    ...
    await supa.rpc('news_add', { p_kind: 'pvp', p_title: title, p_body: body, p_target: defenderId })
...
// 🔒 Safe fallback wrapper so callers don’t hard-crash when RPC is missing
export async function clansLeaderboard(limit = 50) {
  ...
  if (r.error) {
    ...
    return [] as any[]; // or fallback to profiles leaderboard if you add one
  }
  return r.data ?? [];
}
