...
import type { Profile, APStatus, Job, UserJob, UpgradeState, PvETier, RaidTarget, BattleResult, UpgradeTrack, Batch } from '@/lib/types';
...
export class MockAPI implements DataAPI {
    ...
    async refillAP(): Promise<APStatus> {
        if (!MOCK_DB.profile?.premiumFeatures?.apRefill) {
            throw new Error("Premium feature required.");
        }
        MOCK_DB.ap.apNow = MOCK_DB.ap.apMax;
        return this.delay({ ...MOCK_DB.ap });
    }
    ...
    // FIX: Implemented missing method getActiveJob.
    ...
    // FIX: Implemented missing method getLeaderboard.
